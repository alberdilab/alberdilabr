...
The documents you write sit in the project root. Everything the render needs but
nobody edits day to day is filed under `alberdilabr/`.

```text
my-analysis/
├── index.Rmd              landing page, shared YAML front matter, output format
├── 01-introduction.Rmd
├── 02-methods.Rmd
├── 03-results.Rmd
├── 04-discussion.Rmd
...
`_bookdown.yml` stays in the root because bookdown looks for it there: keeping
it in place is what lets `bookdown::render_book()` build the project with no
arguments, from RStudio's *Build Book* button as readily as from CI.
...
Chapters are numbered `.Rmd` files in the project root, listed explicitly in
